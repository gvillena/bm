...
export function usePresenceHint({ directives, budget = 2 }: UsePresenceHintOptions) {
  return useMemo(() => {
    if (!directives) {
      return {
        tone: undefined,
    ...
    const hints = (directives.hints ?? []).slice(0, budget).map((hint, index) => {
      const sanitized = sanitizeRichText(hint);
      const id = `aria-hint-${index}`;
      return { id, text: sanitized, describedBy: `${id}-desc` } satisfies PresenceHint;
    });

    const actions: PresenceAction[] = (directives.actions ?? []).slice(0, 2).map((action) => ({
      ...action,
      sanitizedLabel: sanitizeRichText(action.label)
    }));

    const explain = directives.explain ? sanitizeRichText(directives.explain) : undefined;
