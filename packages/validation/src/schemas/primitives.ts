...
import { boundedString, safeText as safeTextValidator } from "../validators/strings";
...
type IdBrand<T extends string> = Brand<`id:${T}`>;

export type Id<T extends string> = Branded<string, `id:${T}`>;

export const IdSchema = <T extends string>(resource: T) =>
  z
    .string()
    .refine((value) => value.length === 36 || value.length === 24, { message: idMessage(resource) })
    .superRefine((value, ctx) => {
      if (value.length === 36 && !z.string().uuid().safeParse(value).success) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: idMessage(resource) });
      }
...
export const Tier = z.enum(["FREE", "CIRCLE", "ALQUIMIA", "SUPER_ALQUIMIA"] as const);
...
export const VerificationLevel = z.enum(["0", "1", "2"], {
  required_error: "Nivel de verificación requerido"
